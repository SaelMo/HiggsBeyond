...
C     --=========================================--
      ...
      SUBROUTINE ML5_0_0_1_SLOOPMATRIX(P_USER,ANS)
C     
C     Generated by MadGraph5_aMC@NLO v. 2.7.3, 2020-06-21
...
C     
C     Process: g g > h h h WEIGHTED<=18 [ noborn = QCD ]
C     
C     Modules
C     
...
C     DIRECT_ME_COMPUTATION    = Each loop amplitude is squared
C      individually against all amplitudes with its own color factor.
C     ME_COMPUTATION_FROM_JAMP = Amplitudes are first projected onto
C      color flows (many less of them) which are then squared to form
C      the ME.
C     When setting both computation method to .TRUE., their systematic
C      comparisons will be printed out.
      DATA DIRECT_ME_COMPUTATION/.FALSE./
C     When using this MadLoop output for integration with MadEvent,
C      ME_COMPUTATION_FROM_JAMP *must* be set to .True. because it is
C      necessary to compute the AMP2 setting up the multichanneling. 
...
C      provide QP
      ...
      COMMON/ML5_0_0_1_STABILITY_TESTS/MAXSTABILITYLENGTH
...
C      comparisons is inferred from the first 100 points at most
C      (notice that the weight of a  given kinematic configuration can
C      appear more than once because of the stability tests).
C     When changing this parameter, make sure to correspondingly
C      update the parameter with the same name in MadLoopCommons.f.
      INTEGER MAXNREF_EVALS
      PARAMETER (MAXNREF_EVALS=100)
      REAL*8 REF_EVALS(MAXNREF_EVALS)
...
C      DIFFERENT EVALUATION METHODS IN ORDER TO ASSESS STABILITY.
     ...
     $ /NSQSOXNLG*.FALSE./
      ...
      COMMON/ML5_0_0_1_RUNTIME_OPTIONS/AUTOMATIC_CACHE_CLEARING
...
C      stability check to take place
      ...
      COMMON/ML5_0_0_1_BYPASS_CHECK/BYPASS_CHECK, ALWAYS_TEST_STABILITY
C     
C     FUNCTIONS
C     
      INTEGER ML5_0_0_1_TIRCACHE_INDEX
      INTEGER ML5_0_0_1_ML5SOINDEX_FOR_BORN_AMP
      INTEGER ML5_0_0_1_ML5SOINDEX_FOR_LOOP_AMP
      INTEGER ML5_0_0_1_ML5SQSOINDEX
      INTEGER ML5_0_0_1_ISSAME
      LOGICAL ML5_0_0_1_ISZERO
      LOGICAL ML5_0_0_1_IS_HEL_SELECTED
...
C     
      ...
      COMMON/ML5_0_0_1_DOUBLECHECK_JAMP/RES_FROM_JAMP
     $ ,DIRECT_ME_COMPUTATION,ME_COMPUTATION_FROM_JAMP
      ...
      DATA N_DP_EVAL/1/
      DATA N_QP_EVAL/1/
      COMMON/ML5_0_0_1_N_EVALS/N_DP_EVAL,N_QP_EVAL
...
C      AVAILABLE OR NOT
     ...
     $ ,.TRUE.,.TRUE./
      COMMON/ML5_0_0_1_LOOPLIBS_AV/ LOOPLIBS_AVAILABLE
...
C      FOR STABILITY PURPOSE
      ...
      REAL*8 LSCALE
      INTEGER CTMODE
      COMMON/ML5_0_0_1_CT/LSCALE,CTMODE
      LOGICAL MP_PS_SET
...
C     Typically, the hel_offset is 10000
      ...
      INTEGER HELPICKED
      DATA HELPICKED/-1/
      COMMON/ML5_0_0_1_HELCHOICE/HELPICKED
      INTEGER USERHEL
      DATA USERHEL/-1/
      COMMON/ML5_0_0_1_USERCHOICE/USERHEL
...
C      subroutine of MadLoopCommons.dat
      ...
      COMMON/ML5_0_0_1_LOOP_TOOLS/QP_TOOLS_AVAILABLE,INDEX_QP_TOOLS

      COMPLEX*16 W(20,NWAVEFUNCS)
      COMMON/ML5_0_0_1_W/W

      COMPLEX*32 MPW(20,NWAVEFUNCS)
      COMMON/ML5_0_0_1_MP_W/MPW
...
C      coefficients when changing the CTMode for the stability test.
      ...
      COMMON/ML5_0_0_1_SKIP_COEFS/SKIP_LOOPNUM_COEFS_CONSTRUCTION

      LOGICAL TIR_DONE(NLOOPGROUPS)
      COMMON/ML5_0_0_1_TIRCACHING/TIR_DONE
      ...
      COMMON/ML5_0_0_1_LOOPRES/LOOPRES,S

      INTEGER CF_D(NCOLORROWS,NLOOPAMPS)
      INTEGER CF_N(NCOLORROWS,NLOOPAMPS)
      COMMON/ML5_0_0_1_CF/CF_D,CF_N

      INTEGER HELC(NEXTERNAL,NCOMB)
      COMMON/ML5_0_0_1_HELCONFIGS/HELC

      REAL*8 PREC,USER_STAB_PREC
      DATA USER_STAB_PREC/-1.0D0/
      COMMON/ML5_0_0_1_USER_STAB_PREC/USER_STAB_PREC
...
C     RETURNCODE=100*RET_CODE_H+10*RET_CODE_T+RET_CODE_U
      ...
      COMMON/ML5_0_0_1_ACC/ACCURACY,RET_CODE_H,RET_CODE_T,RET_CODE_U

      LOGICAL MP_DONE_ONCE
      DATA MP_DONE_ONCE/.FALSE./
      COMMON/ML5_0_0_1_MP_DONE_ONCE/MP_DONE_ONCE
...
C      reading the parameters
     ...
     $ ,FORCED_CHOICE_OF_COLLIER_IR_POLE_COMPUTATION/.FALSE.,.FALSE./
      COMMON/ML5_0_0_1_COLLIERPOLESFORCEDCHOICE
...
C      independently by each SubProcess.
      ...
      COMMON/ML5_0_0_1_FPE_IN_REDUCTION/FPE_IN_DP_REDUCTION,
     $  FPE_IN_QP_REDUCTION
...
C      requirement.
      ...
      COMMON/ML5_0_0_1_BEAM_POL/POLARIZATIONS
...
C        polarized
     ...
     $       //' setting HELICITYFILTERLEVEL to 1.'
        ...
        IF(.NOT.LOOPINITSTARTOVER) THEN
          WRITE(*,*) '##INFO: For loop-induced processes it is'
     $     //' preferable to always set the parameter'
     $     //' LoopInitStartOver to True, so it is hard-set here to'
     $     //' True.'
          LOOPINITSTARTOVER=.TRUE.
        ENDIF
        IF(.NOT.HELINITSTARTOVER) THEN
          WRITE(*,*) '##INFO: For loop-induced processes it is'
     $     //' preferable to always set the parameter HelInitStartOver'
     $     //' to True, so it is hard-set here to True.'
          HELINITSTARTOVER=.TRUE.
        ENDIF
        IF (CHECKCYCLE.LT.5) THEN
          WRITE(*,*) '##INFO: Due to the dynamic setting of the'
     $     //' reference scale for contributions comparisons, it is'
     $     //' preferable to set the parameter CheckCycle to a value'
     $     //' larger than 4, so it is hard-set here to 5.'
          CHECKCYCLE=5
        ENDIF
...
C       Setup the file paths
        ...
        CALL ML5_0_0_1_SET_N_EVALS(N_DP_EVAL,N_QP_EVAL)
...
C         We write a dummy filter for structural reasons here
 ...
 104    CONTINUE
     ...
     $   //' ML5_0_0_1_ColorNumFactors.dat. File not found'
 105    CONTINUE
        CLOSE(1)
 ...
 106    CONTINUE
        STOP 'Color factors could not be initialized from file'
     $   //' ML5_0_0_1_ColorDenomFactors.dat. File not found'
 107    CONTINUE
        CLOSE(1)
 ...
 108    CONTINUE
     ...
     $   //' from file ML5_0_0_1_HelConfigs.dat. File not found'
 109    CONTINUE
        CLOSE(1)
...
C     For loop-induced processes, we should make sure not to use the
C      first points
C     to set the filters because it doesn't have a reasonable REF
C      scale yet.
...
C      recomputes the same PS point with a different CTMode
 300  CONTINUE
C     Of course the trick of reusing coefficients when reducing at the
C      amplitude level only works when computing one helicity at a time
      IF (USERHEL.NE.-1) THEN
        SKIP_LOOPNUM_COEFS_CONSTRUCTION = .TRUE.
      ENDIF
 308  CONTINUE
C     We don't want to re-initialized the following quantities when
C      checking the helicity filter. (which jumps to label 205 to
C      probe each helicity).
C     We however want to re-initialize them for each new computation
C      part of the stability check (which jumps to label 200)
C     This code is therefore placed before 205 and after 200.
      CALL ML5_0_0_1_REINITIALIZE_CUMULATIVE_ARRAYS()
      IF (ME_COMPUTATION_FROM_JAMP) THEN
C       If both ME computational methods have been used, then the ME
C        computation from color flows was stored in RES_FROM_JAMP and
C        we must reset it here.
        DO I=0,NSQUAREDSO
          DO K=0,3
        ...
        ENDDO
      ENDIF
      IF ((.NOT.DIRECT_ME_COMPUTATION).AND.ME_COMPUTATION_FROM_JAMP)
     $  THEN
C       When computing the ME with color flows, the Born ME will be
C        computed as well, so we reset here the result obtained from
C        the smatrix call above.
        DO I=0,NSQUAREDSO
          ANS(0,I)=0.0D0
        ENDDO
      ENDIF
...
C      precision or when TIR_CACHE_SIZE<2.
     ...
     $ .NE.5).AND.(ML5_0_0_1_TIRCACHE_INDEX(CTMODE).EQ.(TIR_CACHE_SIZE
     $ +1))) THEN
        CALL ML5_0_0_1_CLEAR_TIR_CACHE()
      ENDIF
...
C      to the computation of the next helicity.
 205  CONTINUE
      ...
      IF (.NOT.MP_PS_SET.AND.(CTMODE.EQ.0.OR.CTMODE.GE.4)) THEN
        CALL ML5_0_0_1_SET_MP_PS(P_USER)
        MP_PS_SET = .TRUE.
      ENDIF
...
C     Check if we directly go to multiple precision
      IF (CTMODE.GE.4) THEN
        CALL ML5_0_0_1_MP_COMPUTE_LOOP_COEFS(MP_P,BUFFR_BIS)
        IF ((.NOT.DIRECT_ME_COMPUTATION).AND.ME_COMPUTATION_FROM_JAMP)
     $    THEN
C         If the ME's are computed from the color flows only, we must
C          update the NLO part of ANS from BUFFR_BIS and the Born part
C          of ANS using RES_FROM_JAMP(0,*)
          DO I=0,NSQUAREDSO
          ...
          ENDDO
        ENDIF
C       We must skip the double precision computation of both loop
C        amplitudes and CT amplitudes because they will all be
C        computed in MP_COMPUTE_LOOP_COEFS.
        GOTO 301
      ENDIF
...
C         Handle the possible requirement of specific polarizations
     ...
     $     .EQ.0.AND.(.NOT.ML5_0_0_1_IS_HEL_SELECTED(H))) THEN
            CYCLE
          ENDIF
...
C          associated to given loops
          CALL ML5_0_0_1_HELAS_CALLS_AMPB_1(P,NHEL,H,IC)
 2000     CONTINUE
          CT_REQ_SO_DONE=.TRUE.
...
C         FeynRules, there are none of these type of counterterms.
 ...
 3000     CONTINUE
          ...
          CALL ML5_0_0_1_COEF_CONSTRUCTION_1(P,NHEL,H,IC)
 4000     CONTINUE
          LOOP_REQ_SO_DONE=.TRUE.

          IF(SKIPLOOPEVAL.OR.(.NOT.LOOP_REQ_SO_DONE.AND..NOT.MP_LOOP_RE
     $Q_SO_DONE)) THEN
            GOTO 5000
          ENDIF
          DO I=1,NSQUAREDSO
            ...
            ENDDO
          ENDDO
C         We need the dummy argument I_SO for the squared order index
C          to conform to the structure that the call to the LOOP*
C          subroutine takes for processes with Born diagrams.
          I_SO=1
          CALL ML5_0_0_1_LOOP_CT_CALLS_1(P,NHEL,H,IC)
 5000     CONTINUE
          ...
          IF (DIRECT_ME_COMPUTATION) THEN
            DO I=1,NLOOPAMPS
              DO J=1,NLOOPAMPS
                ...
                ITEMP =
     $            ML5_0_0_1_ML5SQSOINDEX(ML5_0_0_1_ML5SOINDEX_FOR_LOOP_
     $AMP(I),ML5_0_0_1_ML5SOINDEX_FOR_LOOP_AMP(J))
                TEMP2(1) = HEL_MULT*DBLE(CFTOT*(AMPL(1,I)
     $           *DCONJG(AMPL(1,J))))
...
C               It is however a good cross-check.
                TEMP2(2) = HEL_MULT*DBLE(CFTOT*(AMPL(2,I)
     $           *DCONJG(AMPL(1,J)) + AMPL(1,I)*DCONJG(AMPL(2,J))))
                TEMP2(3) = HEL_MULT*DBLE(CFTOT*(AMPL(3,I)
     $           *DCONJG(AMPL(1,J)) + AMPL(1,I)*DCONJG(AMPL(3,J))
     $           +AMPL(2,I)*DCONJG(AMPL(2,J))))
C               To mimick the structure of the squared amplitude
C                reduction, we add here the squared counterterm
C                contribution directly to the result ANS() and put the
C                loop contributions in the LOOPRES array which will be
...
C          if the computation is only done from the color flows
     ...
     $     .EQ.-1.OR.ML5_0_0_1_IS_HEL_SELECTED(H)))) THEN
...
C            argument of the subroutine below).
            CALL ML5_0_0_1_COMPUTE_COLOR_FLOWS(HEL_MULT)
            IF(ME_COMPUTATION_FROM_JAMP) THEN
              CALL ML5_0_0_1_COMPUTE_RES_FROM_JAMP(BUFFRES,HEL_MULT)
              IF(((.NOT.DIRECT_ME_COMPUTATION)
...
C                out this helicity
     ...
     $           -1.OR.ML5_0_0_1_IS_HEL_SELECTED(H)) THEN
...
C                out this helicity
     ...
     $           -1.OR.ML5_0_0_1_IS_HEL_SELECTED(H)) THEN
                  CALL
     $              ML5_0_0_1_COMPUTE_COLOR_FLOWS_DERIVED_QUANTITIES(HE
     $L_MULT)
                ENDIF
              ENDIF
            ENDIF
      ...
      IF(DIRECT_ME_COMPUTATION) THEN
C       Lines below are not necessary when computing the ME from color
C        flows
        DO I=0,NSQUAREDSO
          DO J=1,3
...
C     MadLoop jumps to this label just after having called the
C      subroutine ML5_0_0_1_MP_COMPUTE_LOOP_COEFS to compute OpenLoop
C      coefficients in quadruple precision (and not double precision
C      as done above)
 301  CONTINUE
...
C        flows
      ...
      IF ((.NOT.DIRECT_ME_COMPUTATION).AND.ME_COMPUTATION_FROM_JAMP)
     $  THEN
C       We can skip the update of ANS if it was computed from color
C        flows
        GOTO 1226
      ENDIF
...
C          this helicity
          IF (POLARIZATIONS(0,0).EQ.
     $     -1.OR.ML5_0_0_1_IS_HEL_SELECTED(HELPICKED)) THEN
...
C       We make sure not to perform any check when NTRY is 0 because
C        this means that
C       the REF scale has not been set to the result of the evaluation
C        for the first PS point.
        IF (CHECKPHASE.AND.NTRY.NE.0) THEN
C         SET THE HELICITY FILTER
          IF(.NOT.FOUNDHELFILTER) THEN
            HEL_INCONSISTENT=.FALSE.
            IF(ML5_0_0_1_ISZERO(DABS(HELSAVED(1,HELPICKED))
...
C                 Be looser for helicity check, bring a factor 100
                  DUMMY=ML5_0_0_1_ISSAME(HELSAVED(1,HELPICKED)
     $             ,HELSAVED(1,H),REF,.FALSE.)
                  IF(DUMMY.NE.0) THEN
                    IF(NTRY.EQ.1) THEN
...
C         SET THE LOOP FILTER
          IF(.NOT.FOUNDLOOPFILTER.AND.USELOOPFILTER) THEN
            DO I=1,NLOOPGROUPS
              DO J=1,NSQUAREDSO
                IF(.NOT.ML5_0_0_1_ISZERO(ABS(LOOPRES(1,J,I))
     $           +ABS(LOOPRES(2,J,I))+ABS(LOOPRES(3,J,I)),(REF*1.0D-4)
     $           ,I,J)) THEN
                  IF(NTRY.EQ.1) THEN
                    GOODAMP(J,I)=.TRUE.
...
C         DOUBLE CHECK THE HELICITY FILTER
          IF (GOODHEL(HELPICKED).EQ.-HELOFFSET) THEN
            IF (.NOT.ML5_0_0_1_ISZERO(DABS(HELSAVED(1,HELPICKED))
 ...
 348          CONTINUE
          ...
          IF (GOODHEL(HELPICKED).LT.-HELOFFSET.AND.NTRY.NE.0) THEN
            IF(ML5_0_0_1_ISSAME(HELSAVED(1,HELPICKED),HELSAVED(1
...
C          the reference scale for comparisons.
C         It might be that when asking for specific helicities, the
C          user started with a vanishing helicity
C         not filtered yet. In this case, the new ref would remain
C          zero. So we want to check for this
C         and wait for a point for which the evaluation isn't be zero.
     ...
     $         //' in ',MAXATTEMPTS,' trials'
        ...
        ENDIF
      ELSE
C       When not in checking mode, update the ref for the first
C        MAXNREF_EVALS points (The ref. scale could still be used
C        after this stage if BYPASS_CHECK was set to true at some
C        point using SLOOPMATRIX_THRES).
C       Is is possible to simply remove the if statement below to have
C        a running reference scale which always depends on the
C        MAXNREF_EVALS *last* evaluations. 
...
C     When computing the ME from the color flows, we also compute the
C      born ME from them, so we must apply the normalization factors
C      to the born ME as well.
      IF(((.NOT.DIRECT_ME_COMPUTATION).AND.ME_COMPUTATION_FROM_JAMP))
     $  THEN
        ITEMP=0
      ELSE
...
C         We should finish by the summed orders
          ...
          IF (I.EQ.0) THEN
            WRITE(*,*) ' > Checking the sum of all chosen squared'
     $       //' split orders'
          ELSE
            ...
            ELSE
              DO L=1,NINITIAL
                IF (POLARIZATIONS(L,0).NE.-1) THEN
                  RES_FROM_JAMP(K,I)=RES_FROM_JAMP(K,I)
     $             *BEAMS_HELAVGFACTOR(L)
                  RES_FROM_JAMP(K,I)=RES_FROM_JAMP(K,I)
     $             /POLARIZATIONS(L,0)
                ENDIF
              ENDDO
            ...
            ELSE
              TMPR = ABS((ANS(K,I)-RES_FROM_JAMP(K,I))/((ANS(K,I)
     $         +RES_FROM_JAMP(K,I))/2.0D0))
            ENDIF
...
C            answer from mode 1 and carry on.
     ...
     $   .OR.((.NOT.DOING_QP_EVALS).AND.NROTATIONS_DP.GE.1)) ) THEN
          EVAL_DONE(3)=.TRUE.
          CALL ML5_0_0_1_ROTATE_PS(PS,P,1)
          IF (DOING_QP_EVALS) CALL ML5_0_0_1_MP_ROTATE_PS(MP_PS,MP_P,1)
          GOTO 200
        ENDIF
     ...
     $   .OR.((.NOT.DOING_QP_EVALS).AND.NROTATIONS_DP.GE.2)) ) THEN
          EVAL_DONE(4)=.TRUE.
          CALL ML5_0_0_1_ROTATE_PS(PS,P,2)
          IF (DOING_QP_EVALS) CALL ML5_0_0_1_MP_ROTATE_PS(MP_PS,MP_P,2)
          GOTO 200
        ENDIF

        CALL ML5_0_0_1_ROTATE_PS(PS,P,0)
        IF (DOING_QP_EVALS) CALL ML5_0_0_1_MP_ROTATE_PS(MP_PS,MP_P,0)

C       END OF THE DEFINITIONS OF THE DIFFERENT EVALUATION METHODS

        IF(DOING_QP_EVALS.AND.LOOPLIBS_QPAVAILABLE(MLREDUCTIONLIB(I_LIB)
     $   )) THEN
          CALL ML5_0_0_1_COMPUTE_ACCURACY(QP_RES,N_QP_EVAL,ACC,ANS)
...
C          accuracies to 1.0
     ...
     $         ,.FALSE.)
              NEPS=NEPS+1
              CALL ML5_0_0_1_COMPUTE_ACCURACY(DP_RES,N_DP_EVAL,TEMP1
     $         ,TEMP)
              CALL ML5_0_0_1_COMPUTE_ACCURACY(QP_RES,N_QP_EVAL,ACC,ANS)
              IF(NEPS.LE.10) THEN
...
C              flags.
        ...
        ELSEIF(.NOT.DOING_QP_EVALS)THEN
          CALL ML5_0_0_1_COMPUTE_ACCURACY(DP_RES,N_DP_EVAL,ACC,ANS)
C         If a floating point exception was encountered during the
C          reduction,
C         the result cannot be trusted at all and we hardset all
C          accuracies to 1.0
...
C              flags.
     ...
     $             //' encountered.'
                ENDIF
                CALL ML5_0_0_1_COMPUTE_ACCURACY(DP_RES,N_DP_EVAL,ACC
     $           ,ANS)
                IF (NSQUAREDSO.NE.1) THEN
                  WRITE(*,*) 'Accuracies for each split order,'
     $             //' starting with the summed case'
...
C     Now make sure to turn off the global COLLIER cache if applicable
      CALL ML5_0_0_1_SET_COLLIER_GLOBAL_CACHE(.FALSE.)

      END

      SUBROUTINE ML5_0_0_1_CLEAR_CACHES()
C     
...
C     
      ...
      CALL ML5_0_0_1_CLEAR_COLLIER_CACHE()
      END
...
C     --=========================================--
      ...
      LOGICAL FUNCTION ML5_0_0_1_IS_HEL_SELECTED(HELID)
      IMPLICIT NONE
C     
C     CONSTANTS
C     
...
C     ----------
      ...
      ML5_0_0_1_IS_HEL_SELECTED = .TRUE.
      ...
      DO I=1,NEXTERNAL
        ...
        ENDDO
        IF(.NOT.FOUNDIT) THEN
          ML5_0_0_1_IS_HEL_SELECTED = .FALSE.
          RETURN
        ENDIF
      ENDDO
      RETURN
      ...
      LOGICAL FUNCTION ML5_0_0_1_ISZERO(TOTEST, REFERENCE_VALUE, LOOP,
     $  SOINDEX)
      IMPLICIT NONE
C     
C     CONSTANTS
...
C     ----------
      IF(ABS(REFERENCE_VALUE).EQ.0.0D0) THEN
        ML5_0_0_1_ISZERO=.FALSE.
        WRITE(*,*) '##E02 ERRROR Reference value for comparison is'
     $   //' zero.'
        STOP 1
      ELSE
        ML5_0_0_1_ISZERO=((ABS(TOTEST)/ABS(REFERENCE_VALUE))
     $   .LT.ZEROTHRES)
      ENDIF

      IF(LOOP.NE.-1) THEN
        IF((.NOT.ML5_0_0_1_ISZERO).AND.(.NOT.S(SOINDEX,LOOP))) THEN
     ...
     $     ,(ABS(TOTEST)/ABS(REFERENCE_VALUE)),' but is unstable.'
      ...
      INTEGER FUNCTION ML5_0_0_1_ISSAME(RESA,RESB,REF,USEMAX)
      IMPLICIT NONE
...
C      inconsistency, then we only allow for weight one comparisons.
      ...
      DO I=1,N_WGT_TO_TRY
        DO J=1,3
          IF (ML5_0_0_1_ISZERO(ABS(RESB(J)),REF,-1,-1)) THEN
            IF(.NOT.ML5_0_0_1_ISZERO(ABS(RESB(J))+ABS(RESA(J)),REF,-1,
     $       -1)) THEN
              GOTO 1231
            ENDIF
C           Be looser for helicity comparison, so bring a factor 100
          ELSEIF(.NOT.ML5_0_0_1_ISZERO(ABS((RESA(J)/RESB(J))
     $     -DBLE(WGT_TO_TRY(I))),1.0D0,-1,-1)) THEN
            GOTO 1231
          ENDIF
        ENDDO
        ML5_0_0_1_ISSAME = WGT_TO_TRY(I)
        RETURN
 1231   CONTINUE
      ENDDO
      END

      SUBROUTINE ML5_0_0_1_COMPUTE_ACCURACY(FULLLIST, LENGTH, ACC,
     $  ESTIMATE)
      IMPLICIT NONE
C     
C     PARAMETERS 
...
C        accuracy to 1.0d99.
      ...
      SUBROUTINE ML5_0_0_1_SET_N_EVALS(N_DP_EVALS,N_QP_EVALS)
...
C      VARIABLES FROM A GIVEN VARIABLE IN DOUBLE PRECISION
      SUBROUTINE ML5_0_0_1_SET_MP_PS(P)
      ...
      DO I=1,NEXTERNAL
        ...
        ENDDO
      ENDDO
      CALL ML5_0_0_1_MP_IMPROVE_PS_POINT_PRECISION(MP_PS)
      DO I=1,NEXTERNAL
        DO J=0,3
          MP_P(J,I)=MP_PS(J,I)
        ENDDO
...
C     --=========================================--
      ...
      SUBROUTINE ML5_0_0_1_GET_NSQSO_LOOP(NSQSO)
C     
...
C     
      ...
      SUBROUTINE ML5_0_0_1_GET_ANSWER_DIMENSION(ANS_DIM)
C     
...
C     
      ...
      INTEGER FUNCTION ML5_0_0_1_ML5SOINDEX_FOR_SQUARED_ORDERS(ORDERS)
C     
...
C     
      ...
      COMMON/ML5_0_0_1_ML5SQPLITORDERS/SQPLITORDERS
C     
C     BEGIN CODE
C     
      DO I=1,NSQSO
        ...
        ENDDO
        ML5_0_0_1_ML5SOINDEX_FOR_SQUARED_ORDERS = I
        RETURN
 1009   CONTINUE
      ENDDO

      WRITE(*,*) 'ERROR:: Stopping function'
     $ //' ML5_0_0_1_ML5SOINDEX_FOR_SQUARED_ORDERS'
      WRITE(*,*) 'Could not find squared orders ',(ORDERS(I),I=1,NSO)
      STOP

      END

      INTEGER FUNCTION ML5_0_0_1_ML5SOINDEX_FOR_BORN_AMP(AMPID)
C     
...
C     -----------
     ...
     $   //' maximum ',NBORNAMPS
      ENDIF
      ML5_0_0_1_ML5SOINDEX_FOR_BORN_AMP = BORNAMPORDERS(AMPID)

      END

      INTEGER FUNCTION ML5_0_0_1_ML5SOINDEX_FOR_LOOP_AMP(AMPID)
C     
...
C     
      ...
      DATA (LOOPAMPORDERS(I),I= 21, 25) /    1,    1,    1,    1,    1/
      DATA (LOOPAMPORDERS(I),I= 26, 30) /    1,    1,    1,    1,    1/
      DATA (LOOPAMPORDERS(I),I= 31, 35) /    1,    1,    1,    1,    1/
      DATA (LOOPAMPORDERS(I),I= 36, 40) /    1,    1,    1,    1,    1/
      DATA (LOOPAMPORDERS(I),I= 41, 45) /    1,    1,    1,    1,    1/
      DATA (LOOPAMPORDERS(I),I= 46, 50) /    1,    1,    1,    1,    1/
      DATA (LOOPAMPORDERS(I),I= 51, 55) /    1,    1,    1,    1,    1/
      DATA (LOOPAMPORDERS(I),I= 56, 60) /    1,    1,    1,    1,    1/
      DATA (LOOPAMPORDERS(I),I= 61, 65) /    1,    1,    1,    1,    1/
      DATA (LOOPAMPORDERS(I),I= 66, 70) /    1,    1,    1,    1,    1/
      DATA (LOOPAMPORDERS(I),I= 71, 75) /    1,    1,    1,    1,    1/
      DATA (LOOPAMPORDERS(I),I= 76, 80) /    1,    1,    1,    1,    1/
      DATA (LOOPAMPORDERS(I),I= 81, 85) /    1,    1,    1,    1,    1/
      DATA (LOOPAMPORDERS(I),I= 86, 90) /    1,    1,    1,    1,    1/
      DATA (LOOPAMPORDERS(I),I= 91, 95) /    1,    1,    1,    1,    1/
      DATA (LOOPAMPORDERS(I),I= 96,100) /    1,    1,    1,    1,    1/
      DATA (LOOPAMPORDERS(I),I=101,105) /    1,    1,    1,    1,    1/
      DATA (LOOPAMPORDERS(I),I=106,110) /    1,    1,    1,    1,    1/
      DATA (LOOPAMPORDERS(I),I=111,115) /    1,    1,    1,    1,    1/
      DATA (LOOPAMPORDERS(I),I=116,120) /    1,    1,    1,    1,    1/
      DATA (LOOPAMPORDERS(I),I=121,125) /    1,    1,    1,    1,    1/
      DATA (LOOPAMPORDERS(I),I=126,130) /    1,    1,    1,    1,    1/
      DATA (LOOPAMPORDERS(I),I=131,135) /    1,    1,    1,    1,    1/
      DATA (LOOPAMPORDERS(I),I=136,140) /    1,    1,    1,    1,    1/
      DATA (LOOPAMPORDERS(I),I=141,145) /    1,    1,    1,    1,    1/
      DATA (LOOPAMPORDERS(I),I=146,150) /    1,    1,    1,    1,    1/
      DATA (LOOPAMPORDERS(I),I=151,155) /    1,    1,    1,    1,    1/
      DATA (LOOPAMPORDERS(I),I=156,160) /    1,    1,    1,    1,    1/
      DATA (LOOPAMPORDERS(I),I=161,165) /    1,    1,    1,    1,    1/
      DATA (LOOPAMPORDERS(I),I=166,170) /    1,    1,    1,    1,    1/
      DATA (LOOPAMPORDERS(I),I=171,175) /    1,    1,    1,    1,    1/
      DATA (LOOPAMPORDERS(I),I=176,180) /    1,    1,    1,    1,    1/
      DATA (LOOPAMPORDERS(I),I=181,185) /    1,    1,    1,    1,    1/
      DATA (LOOPAMPORDERS(I),I=186,190) /    1,    1,    1,    1,    1/
      DATA (LOOPAMPORDERS(I),I=191,195) /    1,    1,    1,    1,    1/
      DATA (LOOPAMPORDERS(I),I=196,200) /    1,    1,    1,    1,    1/
      DATA (LOOPAMPORDERS(I),I=201,205) /    1,    1,    1,    1,    1/
      ...
      DATA (LOOPAMPORDERS(I),I=261,265) /    1,    1,    1,    1,    1/
      DATA (LOOPAMPORDERS(I),I=266,270) /    1,    1,    1,    1,    1/
      DATA (LOOPAMPORDERS(I),I=271,275) /    1,    1,    1,    1,    1/
      DATA (LOOPAMPORDERS(I),I=276,280) /    1,    1,    1,    1,    1/
      DATA (LOOPAMPORDERS(I),I=281,285) /    1,    1,    1,    1,    1/
      DATA (LOOPAMPORDERS(I),I=286,290) /    1,    1,    1,    1,    1/
      DATA (LOOPAMPORDERS(I),I=291,295) /    1,    1,    1,    1,    1/
      DATA (LOOPAMPORDERS(I),I=296,300) /    1,    1,    1,    1,    1/
      DATA (LOOPAMPORDERS(I),I=301,305) /    1,    1,    1,    1,    1/
      DATA (LOOPAMPORDERS(I),I=306,310) /    1,    1,    1,    1,    1/
      DATA (LOOPAMPORDERS(I),I=311,315) /    1,    1,    1,    1,    1/
      DATA (LOOPAMPORDERS(I),I=316,320) /    1,    1,    1,    1,    1/
      DATA (LOOPAMPORDERS(I),I=321,325) /    1,    1,    1,    1,    1/
      DATA (LOOPAMPORDERS(I),I=326,330) /    1,    1,    1,    1,    1/
      DATA (LOOPAMPORDERS(I),I=331,335) /    1,    1,    1,    1,    1/
      DATA (LOOPAMPORDERS(I),I=336,340) /    1,    1,    1,    1,    1/
      DATA (LOOPAMPORDERS(I),I=341,345) /    1,    1,    1,    1,    1/
      DATA (LOOPAMPORDERS(I),I=346,350) /    1,    1,    1,    1,    1/
      DATA (LOOPAMPORDERS(I),I=351,354) /    1,    1,    1,    1/
C     -----------
C     BEGIN CODE
C     -----------
     ...
     $   //' maximum ',NLOOPAMPS
      ENDIF
      ML5_0_0_1_ML5SOINDEX_FOR_LOOP_AMP = LOOPAMPORDERS(AMPID)

      END


      INTEGER FUNCTION ML5_0_0_1_ML5SQSOINDEX(ORDERINDEXA, ORDERINDEXB)
C     
...
C     
      ...
      COMMON/ML5_0_0_1_ML5AMPSPLITORDERS/AMPSPLITORDERS
C     
C     FUNCTION
C     
      INTEGER ML5_0_0_1_ML5SOINDEX_FOR_SQUARED_ORDERS
C     
C     BEGIN CODE
C     
      DO I=1,NSO
     ...
     $   +AMPSPLITORDERS(ORDERINDEXB,I)
      ENDDO
      ML5_0_0_1_ML5SQSOINDEX=ML5_0_0_1_ML5SOINDEX_FOR_SQUARED_ORDERS(SQ
     $ORDERS)
      END

C     This is the inverse subroutine of ML5SOINDEX_FOR_SQUARED_ORDERS.
C      Not directly useful, but provided nonetheless.
      SUBROUTINE ML5_0_0_1_ML5GET_SQUARED_ORDERS_FOR_SOINDEX(SOINDEX
     $ ,ORDERS)
C     
...
C     
      ...
      WRITE(*,*) 'ERROR:: Stopping function'
     $ //' ML5_0_0_1_ML5GET_SQUARED_ORDERS_FOR_SOINDEX'
...
C      Not directly useful, but provided nonetheless.
      SUBROUTINE ML5_0_0_1_ML5GET_ORDERS_FOR_AMPSOINDEX(SOINDEX,ORDERS)
C     
...
C     
      ...
      WRITE(*,*) 'ERROR:: Stopping function'
     $ //' ML5_0_0_1_ML5GET_ORDERS_FOR_AMPSOINDEX'
...
C      completeness
      INTEGER FUNCTION ML5_0_0_1_ML5SOINDEX_FOR_AMPORDERS(ORDERS)
C     
...
C     
      DO I=1,NAMPSO
        ...
        ENDDO
        ML5_0_0_1_ML5SOINDEX_FOR_AMPORDERS = I
        RETURN
 1009   CONTINUE
      ENDDO

      WRITE(*,*) 'ERROR:: Stopping function'
     $ //' ML5_0_0_1_ML5SOINDEX_FOR_AMPORDERS'
      WRITE(*,*) 'Could not find squared orders ',(ORDERS(I),I=1,NSO)
      STOP
...
C     --=========================================--
      ...
      SUBROUTINE ML5_0_0_1_COLLIER_COMPUTE_UV_POLES(ONOFF)
C     
...
C      overwrite the choice made here
      ...
      SUBROUTINE ML5_0_0_1_COLLIER_COMPUTE_IR_POLES(ONOFF)
C     
C     This function can be called by the MadLoop user so as to chose
C      to have COLLIER
...
C      overwrite the choice made here
      ...
      SUBROUTINE ML5_0_0_1_FORCE_STABILITY_CHECK(ONOFF)
C     
...
C     
      ...
      SUBROUTINE ML5_0_0_1_SET_AUTOMATIC_CACHE_CLEARING(ONOFF)
C     
...
C     
     ...
     $   //' NRotations_DP and NRotations_QP to 0.'
      ...
      SUBROUTINE ML5_0_0_1_SET_COUPLINGORDERS_TARGET(SOTARGET)
      IMPLICIT NONE
C     
...
C     ----------
      ...
      SUBROUTINE ML5_0_0_1_SET_LEG_POLARIZATION(LEG_ID,
     $  LEG_POLARIZATION)
      IMPLICIT NONE
C     
C     ARGUMENTS
...
C     ----------
      ...
      ELSE
        ...
        ENDDO
      ENDIF
      CALL ML5_0_0_1_SET_LEG_POLARIZATIONS(LEG_ID,LEG_POLARIZATIONS)

      END

      SUBROUTINE ML5_0_0_1_SET_LEG_POLARIZATIONS(LEG_ID,
     $  LEG_POLARIZATIONS)
      IMPLICIT NONE
C     
C     CONSTANTS
...
C     leg is POLARIZATIONS(legID,1: POLARIZATIONS(legID,0)   ).
      ...
      COMMON/ML5_0_0_1_BORN_BEAM_POL/BORN_POLARIZATIONS
...
C     ----------
      ...
      SUBROUTINE ML5_0_0_1_SLOOPMATRIXHEL(P,HEL,ANS)
      IMPLICIT NONE
C     
C     CONSTANTS
C     
...
C     ----------
      USERHEL=HEL
      CALL ML5_0_0_1_SLOOPMATRIX(P,ANS)
      END

      SUBROUTINE ML5_0_0_1_SLOOPMATRIXHEL_THRES(P,HEL,ANS,PREC_ASKED
     $ ,PREC_FOUND,RET_CODE)
      IMPLICIT NONE
C     
C     CONSTANTS
...
C     
      ...
      INTEGER H,T,U
      REAL*8 ACCURACY(0:NSQUAREDSO)
      COMMON/ML5_0_0_1_ACC/ACCURACY,H,T,U
...
C     ----------
      ...
      CALL ML5_0_0_1_SLOOPMATRIXHEL(P,HEL,ANS)
...
C     Reset it to default value not to affect next runs
      ...
      SUBROUTINE ML5_0_0_1_SLOOPMATRIX_THRES(P,ANS,PREC_ASKED
     $ ,PREC_FOUND,RET_CODE)
C     
C     Inputs are:
...
C      de-allocating
C     arrays
      SUBROUTINE ML5_0_0_1_EXIT_MADLOOP()
      CALL ML5_0_0_1_DEALLOCATE_COLOR_FLOWS()
      CONTINUE
      END
